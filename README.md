...
*   Public instance attributes:
    *   `id`: string - assign with an `uuid` when an instance is created:
        *   you can use `uuid.uuid4()` to generate unique `id` but t forget to convert to a string.
...
*   Public instance methods:
    ...
    *   `to_dict(self)`: returns a dictionary containing all keys/values of `__dict__` of the instance:
        ...
        *   `created_at` and `updated_at` must be converted to string object in ISO format:
            ...
            *   This method will be the first piece of the serialization/deserialization process: create a dictionary representation simple object  of our `BaseModel`.
...
s great but s still not persistent: every time you launch the program, you t restore all objects created 
Writing the dictionary representation to a file t be relevant:

*   Python t know how to convert a string to a dictionary (easily).
*   s not human readable.
...
*   Public instance methods:
    ...
    *   `reload(self)`: deserializes the JSON file to `__objects` (only if the JSON file (`__file_path`) exists ; otherwise, do nothing. If the file t exist, no exception should be raised).
...
*   `__init__(self, *args, **kwargs)`:
    *   if s a new instance (not from a dictionary representation), add a call to the method `new(self)` on `storage`.
...
to make your program executable except when imported. Please t add anything around - the Checker t like it otherwise.
...
*   `create`: Creates a new instance of `BaseModel`, saves it (to the JSON file) and prints the `id`. Ex: `$ create BaseModel`.
    ...
    *   If the class name t exist, print  `** class doesn't exist **` (ex: `$ create MyModel`).
*   `show`: Prints the string representation of an instance based on the class name and `id`. Ex: `$ show BaseModel 1234-1234-1234`.
    *   If the class name is missing, print `** class name missing **` (ex: `$ show`).
    *   If the class name t exist, print `** class doesn't exist **` (ex: `$ show MyModel`).
    *   If the `id` is missing, print `** instance id missing **` (ex: `$ show BaseModel`).
    *   If the instance of the class name t exist for the `id`, print `** no instance found **` (ex: `$ show BaseModel 121212`).
*   `destroy`: Deletes an instance based on the class name and `id` (save the change into the JSON file). Ex: `$ destroy BaseModel 1234-1234-1234`.
    *   If the class name is missing, print `** class name missing **` (ex: `$ destroy`).
    *   If the class name t exist, print `** class doesn't exist **` (ex: `$ destroy MyModel`).
    *   If the `id` is missing, print `** instance id missing **` (ex: `$ destroy BaseModel`).
    *   If the instance of the class name t exist for the `id`, print `** no instance found **` (ex: `$ destroy BaseModel 121212`).
*   `all`: Prints all string representation of all instances based or not on the class name. Ex: `$ all BaseModel` or `$ all`.
    *   The printed result must be a list of strings (like the example below).
    *   If the class name t exist, print `** class doesn't exist **` (ex: $ `all MyModel`).
*   `update`: Updates an instance based on the class name and `id` by adding or updating attribute (save the change into the JSON file). Ex: `$ update BaseModel 1234-1234-1234 email "aibnb@mail.com"`.
    ...
    *   If the class name t exist, print `** class doesn't exist **` (ex: `$ update MyModel`).
    *   If the `id` is missing, print `** instance id missing **` (ex: `$ update BaseModel`).
    *   If the instance of the class name t exist for the `id`, print `** no instance found **` (ex: `$ update BaseModel 121212)`.
    *   If the attribute name is missing, print `** attribute name missing **` (ex: `$ update BaseModel existing-id`).
    *   If the value for the attribute name t exist, print `** value missing **` (ex: `$ update BaseModel existing-id first_name`).
    *   All other arguments should not be used (Ex: `$ update BaseModel 1234-1234-1234 email "aibnb@mail.com" first_name "Betty"` = `$ update BaseModel 1234-1234-1234 email "aibnb@mail.com"`).
    *   `id`, `created_at` and `updated_at`  be updated. You can assume they t be passed in the update command.
    *    arguments can be updated: string, integer and float. You can assume nobody will try to update list of ids or datetime.
...
To collaborate, reach me through the email addresses wendymunyasi@gmail.com.interceptshould LetsimpleOnly woncantdoesndoesndoesndoesndoesndoesndoesndoesndoesnwondonshouldnitdoesnItdoesnwonbeforedonitItwonittypewith donwhatit